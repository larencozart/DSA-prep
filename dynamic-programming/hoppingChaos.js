...
// Example 2:
    ...
    // Brute Force Approah (using recursion)
    function hoppingChaos1(n) {
      if (n === 1) return 1;
      if (n === 2) return 2;
    ...
    function hoppingChaos(n) {
      ...
      return waysToN(n, memo);
    }

    // Iterative Solution + array => space O(N)
    function hoppingChaos(n) {
      if (n === 1) return 1;
      if (n === 2) return 2;
      ...
      for (let stack = 3; stack <= n; stack += 1) {
        dp[stack] = dp[stack - 1] + dp[stack - 2];
      }

      return dp[n];
    }

    // Iterative Solution without array => space O(1)
    function hoppingChaos(n) {
      if (n == 1) return 1;
      if (n === 2) return 2;
